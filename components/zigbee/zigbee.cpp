...
void ZigBeeComponent::set_attr(uint8_t endpoint_id, uint16_t cluster_id, uint8_t role, uint16_t attr_id,
  ...
  if (state != ESP_ZB_ZCL_STATUS_SUCCESS) {
    ESP_LOGE(TAG, "Setting attribute failed! status(0x%02x) value(%d) ep: %d cluster_id: 0x%04x attr_id: 0x%04x role: %d", state,  static_cast<int>(reinterpret_cast<intptr_t>(value_p)), endpoint_id, cluster_id, attr_id, role);
    return;
  } else
    ESP_LOGD(TAG, "Attribute set: status(0x%02x) value(%d) ep: %d cluster_id: 0x%04x attr_id: 0x%04x role: %d", state,  static_cast<int>(reinterpret_cast<intptr_t>(value_p)), endpoint_id, cluster_id, attr_id, role);
  esp_zb_lock_release();
}
